...
local({
# Spatial emissions proxy: fuel consumption * burn mask. Uses the same
# IGBP-based fuel lookup as chapter 3 and multiplies by the 25-year burn
# composite. Values in t DM ha⁻¹ of biomass burned.
...
lookup <- matrix(c(
  ...
  6,  50, 7,  40, 8,  20, 9,  15, 10, 10,
  11, NA, 12, NA, 13, NA, 14, NA, 15, NA, 16, NA, 17, NA
), ncol = 2, byrow = TRUE)
fuel <- terra::classify(igbp, lookup, others = NA)
emissions <- fuel * burn

tmap::tmap_mode("view")
tmap::tm_shape(aoi_country_sf) + tmap::tm_borders(col = "white", lwd = 1.5) +
  tm_local_continuous(emissions,
    palette_colours = c("#fdae61", "#f46d43", "#d73027", "#7f0000"),
    title = "Biomass burned (t DM ha⁻¹)") +
