...
Using FAO's dataset of administrative boundaries sourced from the Earth 
Engine Catalog, we derive an example AOI polygon used in all 
...
## Worked Example: Quality Filtering 
...
tmap::tm_basemap("Esri.WorldImagery") +
tmap::tm_tiles(
  ee_tile_url(pre_burn, list(palette = "red", min = 0L, max = 1L)),
  group = "Before Filter") +
tmap::tm_tiles(
  ee_tile_url(post_burn, list(palette = "yellow", min = 0L, max = 1L)),
  group = "After Filter (<20%)") +
tmap::tm_shape(aoi_country_sf) +
  tmap::tm_borders(col = "white", lwd = 1.5) +
tmap::tm_scalebar(
  position = c("RIGHT", "BOTTOM"), text.size = 0.5)
```
