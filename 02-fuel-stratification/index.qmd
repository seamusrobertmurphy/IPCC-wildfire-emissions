...
tmap::tm_shape(aoi_country_sf) + tmap::tm_borders(col = "white", lwd = 1.5) +
  tmap::tm_tiles(
    ee_tile_url(
      forest_mask$selfMask()$clip(aoi_country_ee$geometry()),
      list(palette = "086a10", min = 1L, max = 1L)
    ), group = "Forest (IGBP 1-5)"
  ) +
  tmap::tm_scalebar(position = c("RIGHT", "BOTTOM"), text.size = 0.5) +
  tmap::tm_basemap("Esri.WorldImagery")
```
...
# GLAD canopy height in GEE (Potapov et al.)
...
# Agreement: both say forest
# MODIS-only: MODIS says forest, GLAD does not
# GLAD-only: GLAD says forest, MODIS does not
...
knitr::kable(output_table,
  caption = "Honduras 2010 — Vegetation Classification Output",
  digits = 1)
```
