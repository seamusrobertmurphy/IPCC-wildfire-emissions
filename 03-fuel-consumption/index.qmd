...
local({
# Reclass fuel consumption (t DM ha⁻¹) by IGBP class locally. Values
# from IPCC 2019 Refinement Table 2.4 / 2.5 defaults.
...
lookup <- matrix(c(
  1, 100, 2, 100, 3,  90, 4,  80, 5,  70,   # forest
  6,  50, 7,  40, 8,  20, 9,  15, 10, 10,   # savanna/grass
...
tmap::tm_shape(aoi_country_sf) + tmap::tm_borders(col = "white", lwd = 1.5) +
  tm_local_continuous(fuel,
    palette_colours = c("#fee08b", "#fdae61", "#f46d43", "#d73027"),
...
igbp <- local_raster("mcd12q1_igbp_2020")
savanna <- igbp >= 6 & igbp <= 10

# Early dry season fuel = 40% of peak; late dry season = 80%. Values
# illustrative, matching the original chunk's early/late split.
early <- terra::ifel(savanna, 20, NA)
late  <- terra::ifel(savanna, 40, NA)

tmap::tmap_mode("view")
tmap::tm_shape(aoi_country_sf) + tmap::tm_borders(col = "white", lwd = 1.5) +
  tm_local_continuous(early,
    palette_colours = c("#fff5b8", "#fdae61"),
    title = "Early dry (t DM ha⁻¹)", group = "Early dry season") +
  tm_local_continuous(late,
    palette_colours = c("#fdae61", "#a50026"),
    title = "Late dry (t DM ha⁻¹)", group = "Late dry season") +
  tmap::tm_basemap("Esri.WorldImagery")
})
