...
This chapter describes the estimation of fuel mass available for combustion (M~B~) and combustion factors (C~f~) following IPCC Tier 1 default values. These parameters, combined with burned area and vegetation classifications from Chapters 1-2, determine the total biomass burned in fire events.
...
Where:
-   **M~B~** = Mass of fuel available for combustion (tonnes dry matter ha⁻¹)
-   **C~f~** = Combustion factor (dimensionless, 0-1)
-   **M~B~ × C~f~** = Actual biomass consumed per unit area (tonnes DM ha⁻¹)
...
**Tier 1 Simplification**: M~B~ represents **average fuel consumed across fire severity gradient**, from surface fires (low M~B~) to crown fires (high M~B~).

#### Climate-Dependent Forest Fuel Loads {#sec-fuel-forest-climate}

**Boreal Forests** (41.0 t DM ha⁻¹):
-   Slow decomposition → thick organic layers
...
-   Rapid decomposition → less litter
-   BUT: high biomass density
...
-   Woody component adds coarse fuels
-   BUT: tree canopy reduces grass fuel
...
**Combustion Factor** (C~f~) = Proportion of available fuel actually consumed by fire
...
Late season → Lower moisture × Higher intensity → Higher C~f~
...
**Caveat**: C~f~ = 1.0 applies to **depth of burn**, not total peat column
...
## Chapter Summary and Next Steps {#sec-fuel-summary}
...
3. **Combustion factor** (C~f~) represents proportion of fuel consumed, ranging from 0.29 (early season savanna) to 1.0 (organic soils)

4. **Total biomass burned** calculated as: **Area (ha) × M~B~ (t ha⁻¹) × C~f~**

5. **Tier 2/3 improvements** possible through country-specific data, fire severity mapping, and fuel moisture modeling

### Progression to Chapter 4 {#sec-fuel-to-chapter4-summary}

**Chapter 4: Emission Factors and GHG Calculations** will use fuel consumption estimates to:

1. Apply IPCC emission factors (G~ef~) by gas (CO₂, CH₄, N₂O)
